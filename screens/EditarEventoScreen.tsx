...
interface EventoDetalhes {
  id: number;
  tipo: string;
  localidade: LocalidadeResumo;
  dataInicio: string;
  impacto?: string;
}

type EditarEventoRouteProp = RouteProp<RootStackParamList, 'EditarEvento'>;

const EditarEventoScreen = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const route = useRoute<EditarEventoRouteProp>();
  const { evento } = route.params;

  const [tipo, setTipo] = useState(evento.tipo);
  const [dataInicio, setDataInicio] = useState(evento.dataInicio);
  const [localidadeId, setLocalidadeId] = useState<number>(evento.localidade.id);
  const [impacto, setImpacto] = useState(evento.impacto || '');
  ...
  const handleSalvar = async () => {
    ...
    setLoading(true);
    try {
      await api.put(`/eventos/${evento.id}`, eventoData);
      Alert.alert('Sucesso', 'Evento atualizado com sucesso!');
      navigation.goBack();
    } catch (error: any) {
      console.error("API Error updating evento:", error.response?.data || error.message);
      const errorMessage = error.response?.data?.message || 'Falha ao atualizar evento. Verifique os dados e a conexão.';
