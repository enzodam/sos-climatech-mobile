...
type EditarRecursoRouteProp = RouteProp<RootStackParamList, 'EditarRecurso'>;

const EditarRecursoScreen = () => {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const route = useRoute<EditarRecursoRouteProp>();
  const { recurso } = route.params;

  const [nome, setNome] = useState(recurso.nome);
  const [tipo, setTipo] = useState(recurso.tipo);
  const [quantidade, setQuantidade] = useState(recurso.quantidade.toString());
  const [eventoId, setEventoId] = useState<number>(recurso.eventoId);
  ...
  const handleSalvar = async () => {
    ...
    setLoading(true);
    try {
      await api.put(`/recursos/${recurso.id}`, recursoData);
      Alert.alert('Sucesso', 'Recurso atualizado com sucesso!');
      navigation.goBack();
    } catch (error: any) {
      console.error("API Error updating recurso:", error.response?.data || error.message);
      const errorMessage = error.response?.data?.message || 'Falha ao atualizar recurso. Verifique os dados e a conexão.';
...
export default EditarRecursoScreen;
